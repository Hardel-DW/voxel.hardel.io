...
export default function NeoEnchantmentViewer({ enchant }: { enchant: SectionViewer[] }) {
                        ...
                        {currentEnchantment.addons && (
                            <>
                                ...
                                <a
                                    ...
                                    {translate("datapacks.neoenchant.viewer.yggdrasil_addon")}
                                </a>
                            </>
                        )}
                    </div>
                ...
                <div className="hidden self-center md:grid grid-cols-2 xl:grid-cols-4 gap-4">
                    {enchant.map((item) => (
                        <button
                            type="button"
                            className={cn(
                                "transition-all duration-500 opacity-50 hover:opacity-100 cursor-pointer rounded-2xl",
                                item.id === currentCategory.id && "opacity-100"
                            )}
                            key={item.id}
                    ...
                    {isPageLoaded && currentEnchantment.video ? (
                        <>
                            ...
                            <video
                                ...
                                {translate("datapacks.neoenchant.viewer.video_not_supported")}
                            </video>
                        </>
                ...
                <div className="md:hidden flex z-20">
                    <Select
                        ...
                        <SelectTrigger className="w-full">
                            <SelectValue placeholder={translate("datapacks.neoenchant.viewer.select_enchant")} />
                        </SelectTrigger>
                        <SelectContent>
                            <SelectGroup>
                                {enchant.map((item) => (
                        ...
                        {currentCategory.enchants.map((enchantment) => (
                            <button
                                ...
                                className={cn(
                                    ...
                                    enchantment.id === currentEnchantment.id && "opacity-100"
                                )}
                                style={{ backgroundImage: `url("/images/features/card/star.png")` }}
