...
export default function DebugConfigurator<T>(props: {
    assembly: () => RegistryElement<T>[];
}) {
    const [isDebugging, setIsDebugging] = useState(false);
    const [datapack, setDatapack] = useState<Record<string, RegistryElement<T>[]>>();
    const [code, setCode] = useState<RegistryElement<T>>();
    const [namespaces, setNamespaces] = useState<string[]>([]);
    const { translate } = useTranslate();

    const groupByRegistry = (elements: RegistryElement<T>[]): Record<string, RegistryElement<T>[]> => {
        const groups: Record<string, RegistryElement<T>[]> = {};
    ...
    const handleDebugging = () => {
        const assembleDatapack = props.assembly();
                                            ...
                                            {element
                                                ...
                                                .map((element) => (
                                                    ...
                                                    >
                                                        ...
                                                        <small className="text-xs text-gray-400">{element.identifier.filePath()}</small>
                                                    </div>
                                                ))}
                                        </div>
                                    );
    ...
    return (
        <button
            className="fixed p-2 bottom-36 cursor-pointer right-4 bg-zinc-900 z-50 rounded-xl border-t-2 border-l-2 border-zinc-950 size-12"
            onClick={handleDebugging}
            onKeyDown={handleDebugging}
            type="button"
        >
