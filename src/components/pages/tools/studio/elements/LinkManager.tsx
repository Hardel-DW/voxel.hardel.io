...
export default function LinkManager(): React.ReactElement {
    ...
    return (
        <svg role="presentation" className="absolute origin-top-left overflow-visible pointer-events-none" style={{ zIndex: 1000 }}>
            {links.map((link) => {
                ...
                return <Link key={link.id} startX={startPos.x} startY={startPos.y} endX={endPos.x} endY={endPos.y} />;
            })}
        </svg>
    );
}
