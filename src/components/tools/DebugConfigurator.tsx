...
export default function DebugConfigurator() {
	...
	const [datapack, setDatapack] =
		useState<Record<string, RegistryElement<DataDrivenElement>[]>>();
	...
	const groupByRegistry = (
		elements: RegistryElement<DataDrivenElement>[],
	): Record<string, RegistryElement<DataDrivenElement>[]> => {
	...
	const handleDebugging = () => {
		...
		setNamespaces(
			Array.from(
				new Set(
					assembleDatapack.map((element) => element.identifier.getNamespace()),
				),
			),
		);
	};
							...
							<div key={registry}>
								...
								{namespaces.map((namespace) => {
									if (
										!element.some(
											(element) =>
												element.identifier.getNamespace() === namespace,
										)
									)
										return null;
									...
									return (
										<div
											key={namespace}
											className="grid grid-cols-2 gap-4 pr-4 bg-black/30 rounded-2xl p-4 mb-4"
										>
													...
													>
														...
														<div
															className={cn("text-white", {
																"text-rose-500":
																	code &&
																	element.identifier.equals(code?.identifier),
															})}
														>
