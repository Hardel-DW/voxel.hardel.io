...
export async function parseDatapack<T extends keyof Analysers>(
	...
	const packMcmeta: PackMcmeta = JSON.parse(
		new TextDecoder().decode(packMcmetaFile),
	);
	...
	const namespaces = Object.keys(files)
		...
		.filter(
			(namespace, index, self) =>
				namespace && self.indexOf(namespace) === index,
		);
	...
	const mainRegistry = parseDatapackElement<GetAnalyserMinecraft<T>>(
		files,
		main,
	);
	const tagsRegistry = config.analyser.registries.tags
		? parseDatapackElement<TagType>(files, config.analyser.registries.tags)
		: [];
	...
	if (logFile) {
		...
		logger = new Logger(
			...
			existingLog.date,
			existingLog.version,
			existingLog.isModded,
			existingLog.datapack,
			existingLog,
		);
	}
...
export function parseSpecificElement<T extends keyof Analysers>(
	...
	const dataDrivenElement = readDatapackFile<GetAnalyserMinecraft<T>>(
		files,
		identifier,
	);
	...
	const tagsRegistry = analyserResult.config.analyser.registries.tags
		? parseDatapackElement<TagType>(
				files,
				analyserResult.config.analyser.registries.tags,
			)
		: [];

	const tags = tagsRegistry
		.filter((tag) => isPresentInTag(tag, identifier.toString()))
		.map((tag) => tag.identifier.toString());
