...
import type { FileLog, Log, LogDifference, LogValue } from "@/lib/minecraft/core/engine/migrations/types";
 ...
 */
function differenceToAction(difference: LogDifference, field: string): Action | undefined {
    ...
    switch (fileLog.type) {
        ...
        case "added":
            if (typeof fileLog.value === "object" && fileLog.value !== null) {
                const actions = Object.entries(fileLog.value).map(([field, value]): Action | undefined => {
