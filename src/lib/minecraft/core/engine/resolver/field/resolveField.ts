import type {
	...
	ToggleName,
	Unresolved,
} from "@/lib/minecraft/core/engine/resolver/field/type";
...
export function resolveField<T>(
	...
	if (Array.isArray(value)) {
		return value.map((item) =>
			resolveField(item, toggleSection),
		) as Resolved<T>;
	}
		...
		if (obj.type) {
			switch (obj.type) {
				...
				case "get_toggle_name":
					return resolveToggleField(
						value as unknown as ToggleField | ToggleName,
						obj.type,
						toggleSection,
					) as Resolved<T>;
				default:
...
function resolveToggleField(
	value: ToggleField | ToggleName,
	type: string,
	toggleSection?: ToggleSectionMap,
): string {
	if (!toggleSection || Object.keys(toggleSection).length === 0) {
		throw new Error(
			`Cannot resolve toggle ${type} ${value.group}: no toggle section provided`,
		);
	}
