...
import type {
	Analysers,
	GetAnalyserVoxel,
} from "@/lib/minecraft/core/engine/Analyser";
...
export interface ConfiguratorState<T extends keyof Analysers> {
	...
	setToggleSection: (
		section: Record<string, ToggleSection> | undefined,
	) => void;
...
const createConfiguratorStore = <T extends keyof Analysers>() =>
	create<ConfiguratorState<T>>((set, get) => ({
		...
		handleChange: (action, identifier, value) => {
			...
			const element = identifier
				...
				: state.elements.find(
						(elem) =>
							...
							elem.identifier.equals(state.currentElementId),
					);
			...
			const updatedElement = updateData<T>(
				action,
				element,
			...
			set((state) => ({
				elements: state.elements.map((item) =>
					item.identifier.equals(updatedElement.identifier)
						? updatedElement
						: item,
				),
			}));
